...
The following guide only explains how to include the `phr-design-system` in a new project. For usage information check the [documentation website](http://hotosm.github.io/oam-docs/).  

For information on how to develop the `phr-design-system` checkout the [DEVELOPMENT.md](DEVELOPMENT.md)  
...
```
npm install https://github.com/hotosm/phr-design-system#v0.1.1-alpha
```
...
Check the build system of [OAM docs](https://github.com/hotosm/oam-docs/blob/master/gulpfile.js), a project that uses the `phr-design-system`.
...
```js
import { Dropdown, user } from 'phr-design-system';
```
If you want to minimize bundle size you can also include the components directly.  
Bindings exported from `phr-design-system` are also available in `phr-design-system/assets/scripts`
...
.pipe($.sass({
  ...
  precision: 10,
  includePaths: require('node-bourbon').with('node_modules/jeet/scss', require('phr-design-system/gulp-addons').scssPath)
}))
```

The `phr-design-system` uses **Open Sans** which is available on [Google Fonts](https://goo.gl/FZ0Ave).  
...
@import "phr-design-system";
```

### assets/icons
The `phr-design-system` includes svg icons that are compiled into a webfont and included in the styles.  
...
browserSync({
  ...
  server: {
    ...
    middleware: require('phr-design-system/gulp-addons').graphicsMiddleware(fs) // <<< This line
  }
});
```
*Basically every time there's a request to a path like `/assets/graphics/**`, browserSync will check in the `phr-design-system` folder first. If it doesn't find anything it will look in the normal project's asset folder.*
...
gulp.task('images', function () {
  return gulp.src(['app/assets/graphics/**/*', require('phr-design-system/gulp-addons').graphicsPath + '/**/*'])
    .pipe($.cache($.imagemin({
```
